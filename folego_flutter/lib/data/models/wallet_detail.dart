...
class WalletCardPurchaseLine {
  ...
  factory WalletCardPurchaseLine.fromInstallmentJson(
...
class WalletCardInstallmentLine {
  ...
  bool get isOutstanding {
    ...
    return !closedInstallment.contains(status) &&
        !closedInvoice.contains(invoiceStatus);
  }
...
class WalletDebtInstallment {
  ...
  double get remainingAmount {
    final remaining = plannedAmount - paidAmount;
    return remaining < 0 ? 0 : remaining;
  }

  bool get isOverdue => status == 'overdue';

  factory WalletDebtInstallment.fromJson(Map<String, dynamic> json) {
    return WalletDebtInstallment(
      id: json['id'] as String,
      installmentNumber: (json['installment_number'] as num).toInt(),
...
class WalletInstallmentPosition {
  const WalletInstallmentPosition({
    required this.purchaseId,
    required this.description,
  ...
  double get progress {
    ...
    final completedCount = completed
        ...
        : rawCompleted < 0
            ? 0
            : rawCompleted > totalInstallments
                ? totalInstallments
                : rawCompleted;
    return (completedCount / totalInstallments).clamp(0.0, 1.0).toDouble();
  }

  factory WalletInstallmentPosition.fromPurchaseJson(
    Map<String, dynamic> json,
  ) {
    final card = _map(json['card']) ?? const <String, dynamic>{};
    final category = _map(json['category'] ?? json['categories']);
    final installments = _maps(json['card_installments'])
      ..sort(
        (a, b) => (a['installment_number'] as num)
            .toInt()
            .compareTo((b['installment_number'] as num).toInt()),
      );
    ...
    final valid = installments.where((item) {
      final status = item['status'] as String?;
    ...
    final selected = outstanding.isNotEmpty
        ? outstanding.first
        : valid.isNotEmpty
            ? valid.last
            : null;
    ...
    final current = selected == null
        ? total
        : (selected['installment_number'] as num).toInt();
...
List<WalletInstallmentPosition> sortWalletInstallmentPositions(
  Iterable<WalletInstallmentPosition> items,
) {

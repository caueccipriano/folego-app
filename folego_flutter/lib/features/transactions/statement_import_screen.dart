  ...
  Widget _destinationStep() {
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(0, 12, 0, 32), children: [
      ...
      DropdownButtonFormField<String>(
        ...
        items: _sourceOptions.map((item) => DropdownMenuItem<String>(value: _sourceIdOf(item), child: Text(_sourceName(item), overflow: TextOverflow.ellipsis))).toList(growable: false),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          ...
          Expanded(flex: 3, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (row.reason?.trim().isNotEmpty == true) Padding(padding: const EdgeInsets.only(top: 3), child: Text(row.reason!, style: AppTypography.label(context, fontSize: 11, color: AppColors.secondaryText(brightness)))),
          ])),
...
class _DuplicateBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DecoratedBox(decoration: BoxDecoration(color: color.withValues(alpha: .10), borderRadius: BorderRadius.circular(AppRadii.pill)), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), child: Text(text, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: color))));
  }
}

-- Complete ProjectionEngine coverage for future confirmed movements.
-- Keeps the existing card/recurring/budget semantics and adds:
-- - future confirmed cash income/expense events;
-- - future transfers into protected accounts as reserve transfers;
-- - budget-only and balance-only projections as valid inputs.
...
begin
  ...
  future_cash_events as (
    select
      fe.id,
      fe.event_type,
      fi.effective_date as d,
      date_trunc('month', fi.effective_date)::date as month_start,
      coalesce(fi.category_id, fe.category_id) as category_id,
      coalesce(cat.name, 'Outros') as category_name,
      sum(fi.amount)::numeric as net_amount
    from public.financial_events fe
    join public.financial_impacts fi
      on fi.event_id = fe.id
     and fi.space_id = fe.space_id
    join public.accounts acc
      on acc.id = fi.account_id
     and acc.space_id = fi.space_id
     and acc.active
     and acc.type <> 'benefit'
     and acc.available_for_spending
    left join public.categories cat
      on cat.id = coalesce(fi.category_id, fe.category_id)
     and cat.space_id = fe.space_id
    where fe.space_id = p_space_id
      and fe.status = 'confirmed'
      and fi.dimension = 'cash'
      and fi.effective_date > v_today
      and fi.effective_date <= v_end_date
      and fe.event_type not in ('card_payment', 'debt_payment')
    group by
      fe.id,
      fe.event_type,
      fi.effective_date,
      coalesce(fi.category_id, fe.category_id),
      coalesce(cat.name, 'Outros')
    having sum(fi.amount) <> 0
  ),
  future_cash_monthly as (
    select
      m.month_start,
      coalesce(sum(f.net_amount) filter (
        where f.net_amount > 0
          and f.event_type = 'income'
      ), 0)::numeric as guaranteed_income,
      coalesce(sum(-f.net_amount) filter (
        where f.net_amount < 0
          and f.event_type = 'expense'
      ), 0)::numeric as direct_expenses,
      coalesce(sum(-f.net_amount) filter (
        where f.net_amount < 0
          and f.event_type = 'transfer'
      ), 0)::numeric as reserve_transfers,
      coalesce(sum(f.net_amount) filter (
        where f.net_amount > 0
          and f.event_type <> 'income'
      ), 0)::numeric as other_inflows,
      coalesce(sum(-f.net_amount) filter (
        where f.net_amount < 0
          and f.event_type not in ('expense', 'transfer')
      ), 0)::numeric as other_outflows
    from months m
    left join future_cash_events f on f.month_start = m.month_start
    group by m.month_start
  ),
  future_cash_by_category as (
    select
      f.month_start,
      f.category_id,
      f.category_name,
      sum(-f.net_amount)::numeric as amount
    from future_cash_events f
    where f.net_amount < 0
      and f.event_type = 'expense'
    group by f.month_start, f.category_id, f.category_name
  ),
  ...
  components as (
    select
      ...
      (coalesce(r.guaranteed_income, 0)
        + coalesce(f.guaranteed_income, 0)
      ...
      greatest(
        coalesce(f.reserve_transfers, 0)
      ...
      (coalesce(f.other_inflows, 0)
        + coalesce(p.other_inflows, 0)
        + coalesce(a.other_inflows, 0))::numeric as other_inflows,
      greatest(
        coalesce(f.other_outflows, 0)
    ...
    left join future_cash_monthly f on f.month_start = m.month_start
  ...
  category_rows as (
    ...
    where d.amount <> 0
    union all
    select f.month_start, f.category_name, f.amount
    from future_cash_by_category f
    where f.amount <> 0
    union all
  ...
  has_inputs as (
    select (
      v_current_balance <> 0
      or exists(select 1 from budget_rows)
      or exists(select 1 from future_cash_events)
      or exists(

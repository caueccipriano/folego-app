...
-- rewriting any already-applied migration. Safe on Dev where parts already
-- exist, and required for a fresh repo migration replay.
...
revoke all on function public.get_notification_upcoming_events(uuid,smallint,time,integer,integer) from public, anon;
grant execute on function public.get_notification_upcoming_events(uuid,smallint,time,integer,integer) to authenticated;

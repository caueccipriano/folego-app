-- Applied to Supabase Dev as 20260917122833_reconcile_notification_automation_foundation.
-- Reconciles the concurrent foundation without changing ledger semantics.

create index if not exists notification_preferences_space_id_idx
  on public.notification_preferences(space_id);
...
revoke all on function private.normalize_automation_text(text)
...
revoke all on function private.guard_automation_rule_update()
from public, anon, authenticated;

drop trigger if exists automation_rules_guard_update on public.automation_rules;
...
returns table(
  event_key text, source text, source_id uuid, parent_id uuid,
  title text, subtitle text, due_date date, amount numeric,
  direction text, status text, category_id uuid, account_id uuid,
  card_id uuid, debt_id uuid, overdue boolean, realized boolean,
  recurring boolean, installment_number integer, installment_count integer,
  invoice_id uuid, navigation_target text, day_offset integer,
  cash_obligation boolean, recurrence_kind text,
  scheduled_at timestamptz, space_timezone text
)
language plpgsql
stable
security invoker
...
begin
  ...
  return query
  select
    e.event_key, e.source, e.source_id, e.parent_id, e.title, e.subtitle,
    e.due_date, e.amount, e.direction, e.status, e.category_id, e.account_id,
    e.card_id, e.debt_id, e.overdue, e.realized, e.recurring,
    e.installment_number, e.installment_count, e.invoice_id,
    e.navigation_target, e.day_offset, e.cash_obligation,

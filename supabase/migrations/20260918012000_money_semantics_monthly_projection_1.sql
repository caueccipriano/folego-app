-- Fôlego — Money Semantics + Monthly Projection 1.0
-- ProjectionEngine is canonical server-side domain logic.
-- Simulation inputs are transient and never create financial_events.

create table if not exists public.projection_planned_items (
  ...
  constraint projection_planned_items_dates_check
    check (ends_on is null or ends_on >= starts_on),
  constraint projection_planned_items_category_fk
    foreign key (category_id, space_id)
    references public.categories(id, space_id)
    on delete restrict
);

create index if not exists projection_planned_items_space_dates_idx
  on public.projection_planned_items(space_id, starts_on, ends_on)
  where active;

alter table public.projection_planned_items enable row level security;

revoke all on public.projection_planned_items from anon;
revoke insert, update, delete on public.projection_planned_items from authenticated;
grant select on public.projection_planned_items to authenticated;
grant all on public.projection_planned_items to service_role;

drop policy if exists projection_planned_items_select_member
  on public.projection_planned_items;
create policy projection_planned_items_select_member
  on public.projection_planned_items
  for select
  to authenticated
  using ((select private.is_space_member(space_id)));

CREATE OR REPLACE FUNCTION public.add_projection_planning_items(p_space_id uuid, p_items jsonb)
 RETURNS uuid[]
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if p_items is null
    ...
    raise exception 'planning_items_required';
  end if;

  if jsonb_array_length(p_items) > 20 then
    raise exception 'too_many_planning_items';
  end if;

  for v_item in select value from jsonb_array_elements(p_items)
  loop
    v_component := nullif(btrim(v_item ->> 'component'), '');
    v_frequency := coalesce(nullif(btrim(v_item ->> 'frequency'), ''), 'once');
    v_amount := nullif(v_item ->> 'amount_delta', '')::numeric;
    v_starts := nullif(v_item ->> 'starts_on', '')::date;
    v_ends := nullif(v_item ->> 'ends_on', '')::date;
    v_category_id := nullif(v_item ->> 'category_id', '')::uuid;

    if nullif(btrim(v_item ->> 'name'), '') is null
       or v_component not in (
         'income','direct_expense','recurring_expense','card_installment',
         'debt','reserve','investment','other_inflow','other_outflow'
       )
       or v_frequency not in ('once','monthly','yearly')
       or v_amount is null
       or v_amount = 0
       or v_starts is null
       or (v_ends is not null and v_ends < v_starts) then
      raise exception 'invalid_projection_planning_item';
    end if;
    ...
    if v_category_id is not null and not exists (
      ...
      raise exception 'projection_category_not_found';
    end if;

    insert into public.projection_planned_items(
      space_id,
      name,
      component,
      amount_delta,
    ...
    values(
      ...
      v_ends,
      v_category_id,
      coalesce((v_item ->> 'variable_income')::boolean, false),
      coalesce(nullif(btrim(v_item ->> 'source'), ''), 'planning'),
...
revoke all on function public.add_projection_planning_items(uuid,jsonb)
  from public, anon;
grant execute on function public.add_projection_planning_items(uuid,jsonb)
...
begin
  ...
  recurring_by_category as (
    ...
    where ro.item_type = 'expense'
      and coalesce(ro.account_type, '') <> 'benefit'
    group by ro.month_start, ro.category_id, coalesce(ro.category_name, 'Recorrências')
  ),
  budget_rows as (
    select
      m.month_start,
      ...
      bo.category_name,
      case
        when m.month_start = v_start_month
          then greatest(bo.remaining_amount, 0::numeric)
        else greatest(bo.planned_amount, 0::numeric)
      end::numeric as planned_remaining
    from months m
  ...
  card_monthly as (
    select
      m.month_start,
      coalesce(sum(ir.amount_due), 0)::numeric as card_installments
    from months m
  ...
  category_rows as (
    ...
    union all
    select po.month_start, po.category_name, abs(po.amount_delta)::numeric
    ...
    union all
    select ao.month_start, ao.category_name, abs(ao.amount_delta)::numeric
...
revoke all on function public.get_projection(uuid,integer,jsonb,text[])
  from public, anon;
grant execute on function public.get_projection(uuid,integer,jsonb,text[])
  to authenticated, service_role;

comment on function public.get_projection(uuid,integer,jsonb,text[]) is
'Canonical ProjectionEngine. Calculates chained month-by-month projection in PostgreSQL from current available cash, recurring items, effective budgets, open card invoices, debt installments and projection planning items. Transient simulation adjustments never create financial_events.';

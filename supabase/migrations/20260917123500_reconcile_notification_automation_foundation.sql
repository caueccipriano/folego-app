-- Reconciles the concurrent Notifications/Automation foundation without
-- rewriting any migration already applied to Supabase Dev.
...
begin
  ...
  select
    ...
    greatest(
      (
        greatest(v_today, e.due_date - p_offset_days)::timestamp
        + p_preferred_time
      ) at time zone v_timezone,
      now() + interval '1 minute'
    ) as scheduled_at,
    v_timezone as space_timezone
...
revoke all on function public.get_notification_upcoming_events(
  uuid,smallint,time,integer,integer
) from public, anon;
grant execute on function public.get_notification_upcoming_events(
  uuid,smallint,time,integer,integer
) to authenticated;
